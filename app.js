const STORAGE_KEY = 'proreserv_rapport_v6';
...
const el = {
  ...
  toggleExpensesBtn: document.getElementById('toggleExpensesBtn'),
...
function bindEvents() {
  ...
  el.toggleExpensesBtn.addEventListener('click', toggleExpenses);
...
function toggleExpenses() {
  const visible = el.expensesSection.style.display === 'block';
  el.expensesSection.style.display = visible ? 'none' : 'block';
  el.toggleExpensesBtn.textContent = visible ? '+ Spesen anzeigen' : '− Spesen ausblenden';
}
...
function resetForm() {
  ...
  el.other.value = '0.00';
  el.infoText.value = '';
  el.expensesSection.style.display = 'none';
  el.toggleExpensesBtn.textContent = '+ Spesen anzeigen';
}
...
function renderWeek(grouped) {
  ...
  grouped.forEach(group => {
    ...
    card.innerHTML = `
      <div class="day-head">
        <div>
          <div class="day-title">${group.dayCode}, ${formatDateCH(group.date)}</div>
...
function editEntry(id) {
  ...
  const hasExpenses = entry.breakfast || entry.lunch || entry.dinner || entry.parking || entry.taxi || entry.hotel || entry.other;
  el.expensesSection.style.display = hasExpenses ? 'block' : 'none';
  el.toggleExpensesBtn.textContent = hasExpenses ? '− Spesen ausblenden' : '+ Spesen anzeigen';
...
async function exportCurrentWeekExcel() {
  ...
  headerRow.values = ['Tag', 'Datum', 'Zeit', 'Pause', 'Minuten', 'Kilometer', 'Lieferant', 'Modul', 'Markt', 'Spesen (Essen)', 'Parking', 'Taxen', 'Hotel', 'Sonstige'];
  ...
  grouped.forEach((group) => {
    group.entries.forEach((entry) => {
      ...
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        ...
        cell.alignment = { vertical: 'middle', horizontal: [4, 5, 6].includes(colNumber) ? 'center' : 'left' };
    ...
    totalRow.values = ['Total', '', '', '', round2(totals.minutes), round2(totals.kilometers), '', '', '', round2(totals.mealMoney), round2(totals.parking), round2(totals.taxi), round2(totals.hotel), round2(totals.other)];
    ...
    if (infoLines.length) {
      ...
      c.font = { name: 'Arial', size: 15, color: { argb: 'FF8B0000' } };
      c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFDE2E2' } };
  ...
  weeklyRow.values = ['Wochensumme', '', '', '', round2(weekTotals.minutes), round2(weekTotals.kilometers), '', '', '', round2(weekTotals.mealMoney), round2(weekTotals.parking), round2(weekTotals.taxi), round2(weekTotals.hotel), round2(weekTotals.other)];

  weeklyRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
    cell.font = { name: 'Arial', bold: true };
...
function thinBorder(argb) {
  return { top: { style: 'thin', color: { argb } }, left: { style: 'thin', color: { argb } }, bottom: { style: 'thin', color: { argb } }, right: { style: 'thin', color: { argb } } };
}

function mediumBorder(argb) {
  return { top: { style: 'medium', color: { argb } }, left: { style: 'medium', color: { argb } }, bottom: { style: 'medium', color: { argb } }, right: { style: 'medium', color: { argb } } };
}

function weeklyUnderlineBorder(argb) {
  return { top: { style: 'medium', color: { argb } }, left: { style: 'thin', color: { argb } }, right: { style: 'thin', color: { argb } }, bottom: { style: 'double', color: { argb } } };
}
...
function groupByDate(entries) {
  ...
  entries.forEach(entry => {
    if (!map.has(entry.date)) map.set(entry.date, { date: entry.date, dayCode: entry.day, entries: [] });
...
function calculateTotals(entries) {
  return entries.reduce((acc, entry) => {
    ...
    return acc;
  }, { minutes: 0, kilometers: 0, mealMoney: 0, parking: 0, taxi: 0, hotel: 0, other: 0, get totalMoney() { return this.mealMoney + this.parking + this.taxi + this.hotel + this.other; } });
}

function loadEntries() {
  try { const raw = localStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) : []; } catch (error) { return []; }
}
